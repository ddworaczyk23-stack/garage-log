...
export function VehicleDetail({ id }: Props) {
  ...
  // Only vehicles added via the "Add Car" flow (db/vehicleOnboarding.ts) carry
  // a canonicalVehicleId — the two hand-seeded vehicles don't, so their detail
  // view stays exactly as it was (no external-data sections rendered below).
...
// Shared shell for the two "hydrated after Add Car" cards below: loading spinner
// while status is 'loading' (or the row hasn't arrived yet), a retry-able error
// state, and — once loaded — the caller's rendering plus a source/last-updated
// line. Only rendered for vehicles with a canonicalVehicleId (see above).
